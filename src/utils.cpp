...
QByteArray Utils::loadFile(const QString fileName)
{
    QFile file(fileName);
...
bool Utils::saveFile(const QByteArray data, QString fileName)
{
    if(fileName.isEmpty())
        return false;
    ...
    if(qf.isWritable() == false)
    {
//        addLog("Failed to write to %s",fileName.toLocal8Bit().data());
        qf.close();
        return false;
    }
...
bool Utils::saveFile(const char * data, long size, QString fileName)
{
    return saveFile(QByteArray::fromRawData(data,size),fileName);
}
 ...
 */
bool Utils::gzipCompress(const QByteArray input, QByteArray &output, int level)
{
    // Prepare output
    output.clear();
    ...
    if(input.length())
    {
        ...
        char *input_data = (char*)input.constData();
 ...
 */
bool Utils::gzipDecompress(const QByteArray input, QByteArray &output)
{
    // Prepare output
    output.clear();
...
void Utils::appendLog(QString log)
{
    addLog(log.toLocal8Bit().constData());
}

void * Utils::m_parent = 0;

void Utils::addLog(const char* format,...)
{
    if(m_parent != NULL)
    {
        va_list va;
        ...
        ((MainWindow*)m_parent)->addLog(log);
        va_end(va);
    }
}
